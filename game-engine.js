...
async function loadGame(id) {
    ...
    try {
        ...
        alert("Erreur de chargement du fichier " + id + ".json");
        location.reload(); 
    }
}
    ...
    if (dead) {
        textEl.innerHTML = `
            <div class="end-screen">
                ...
                <p>${scenario._config.deathMessage}</p>
            </div>`;
        choiceEl.innerHTML = `<button class="launch-btn" onclick="location.reload()">RETOURNER À L'ACCUEIL</button>`;
    } else if (!resumeId && (!step.choices || step.choices.length === 0)) {
        textEl.innerHTML = `
            <div class="end-screen">
                <h1 class="end-title victory-title">SUCCÈS</h1>
                <p>${step.text.replace(/\[NAME\]/g, player.name)}</p>
            </div>`;
        choiceEl.innerHTML = `<button class="launch-btn" onclick="location.reload()">RETOURNER À L'ACCUEIL</button>`;
    } else {
        ...
        for (let s in player.stats) { ui += `${scenario._config.statLabels[s] || s}: ${player.stats[s]}% | `; }
