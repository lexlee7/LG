                ...
                <div class="lg:col-span-3 card p-12 text-center flex flex-col justify-center min-h-[450px]">
                    <div id="liar-ui">
                        ...
                        <div class="flex gap-4 max-w-sm mx-auto">
                            <button id="btn-coop" onclick="LiarGame.vote('cooperate')" class="flex-1 bg-green-600 py-6 rounded-2xl font-bold hover:scale-105 transition-all">COOPÉRER</button>
                            <button id="btn-betray" onclick="LiarGame.vote('betray')" class="flex-1 bg-red-600 py-6 rounded-2xl font-bold hover:scale-105 transition-all">TRAHIR</button>
                        </div>
                    </div>
...
// ÉCOUTEUR DE RÉSULTATS (DÉBLOQUE L'INTERFACE)
socket.on('liar_results', (data) => {
    const ui = document.getElementById('liar-ui');
    const res = document.getElementById('liar-res');
    
    // On redonne la main aux joueurs
    if(ui) {
        ui.style.opacity = '1';
        ui.style.pointerEvents = 'auto';
    }

    // Affichage écran de fin si vainqueur
    if (data.winner && res && ui) {
        ui.classList.add('hidden');
        res.classList.remove('hidden');
        document.getElementById('res-title').innerText = "PARTIE TERMINÉE";
        document.getElementById('res-msg').innerText = `${data.winner} a remporté la victoire !`;
    }
});
